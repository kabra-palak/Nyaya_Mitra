  ...
  return (
    <div className="space-y-6">
      <header>
        ...
        <p className="text-slate-500">Browse verified legal professionals.</p>
      </header>
      ...
      <input
        ...
        className="w-full border rounded-lg px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-slate-200"
      />

      {loading && (
        <p className="text-slate-400 text-sm animate-pulse">Loading lawyers...</p>
      )}

      {!loading && filtered.length === 0 && (
        <p className="text-slate-400 text-sm">No lawyers found.</p>
      )}

      <div className="grid grid-cols-1 gap-4">
        {filtered.map(lawyer => (
          <div key={lawyer.id} className="border rounded-lg p-6 space-y-4 bg-white shadow-sm">
            <div className="flex items-start justify-between">
              <div>
                <h2 className="text-lg font-semibold">{lawyer.full_name}</h2>
                {lawyer.city && <p className="text-xs text-slate-400">{lawyer.city}</p>}
              </div>
              {lawyer.verified && (
            ...
            {lawyer.specialization && lawyer.specialization.length > 0 && (
              <div className="flex flex-wrap gap-2">
                {lawyer.specialization.map(s => (
                  <span key={s} className="text-xs bg-slate-100 text-slate-600 px-2 py-1 rounded-md">
                    {s}
                  </span>
                ))}
              </div>
